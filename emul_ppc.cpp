...
static inline uint64 ReadMacInt64(uint32 addr)
	{
...
static inline void WriteMacInt64(uint32 addr, uint64 ll)
	{
		patch_qword(addr, ll);
	}
...
static void dump(void)
{
	...
	msg("r12 %08x  r13 %08x  r14 %08x  r15 %08x\n", r[12], r[13], r[14], r[15]);
	msg("r16 %08x  r17 %08x  r18 %08x  r19 %08x\n", r[16], r[17], r[18], r[19]);
	msg("r20 %08x  r21 %08x  r22 %08x  r23 %08x\n", r[20], r[21], r[22], r[23]);
	msg("r24 %08x  r25 %08x  r26 %08x  r27 %08x\n", r[24], r[25], r[26], r[27]);
	msg("r28 %08x  r29 %08x  r30 %08x  r31 %08x\n", r[28], r[29], r[30], r[31]);
	msg(" lr %08x  ctr %08x   cr %08x  xer %08x\n", lr, ctr, cr, xer);
	msg(" pc %08x fpscr %08x\n", pc, fpscr);
...
bctr_nobranch:
		...
		default:
			msg("Illegal 19 opcode %08x (exop %d) at %08x\n", op, exop, pc-4);
			dump();
			break;
	}
}
...
cntlzw_done:if (op & 1)
		...
		case 339: {	// mfspr
			...
			switch (spr) {
				...
				default:
					msg("Illegal mfspr opcode %08x at %08x\n", op, pc-4);
					dump();
			}
			break;
		}
		...
		case 467: {	// mtspr
			...
			switch (spr) {
				...
				default:
					msg("Illegal mtspr opcode %08x at %08x\n", op, pc-4);
					dump();
			}
			break;
		}
		...
		default:
			msg("Illegal 31 opcode %08x (exop %d) at %08x\n", op, exop, pc-4);
			dump();
			break;
	}
}
...
static void emul59(uint32 op)
{
	...
	{
		default:
			msg("Illegal 59 opcode %08x (exop %d) at %08x\n", op, exop, pc-4);
			dump();
			break;
	}
}
...
static void emul63(uint32 op)
{
	...
	switch (exop)
	{
		...
		default:
			msg("Illegal 63 opcode %08x (exop %d) at %08x\n", op, exop, pc-4);
			dump();
			break;
	}
}
...
void emul_ppc(uint32 start, uint32 stop, uint stack)
{
	r[1] = stack;
	running = true;
...
//if (val != old_val)
//{
//	msg("insn at %08lx changed %08lx->%08lx\n", pc-4, old_val, val);
//	old_val = val;
//}
...
//		msg("%08lx at %08lx\n", op, pc);
		...
		switch (primop)
		{
			...
			case 6:		// SheepShaver extensions
				msg("Extended opcode %08x at %08x (68k pc %08x)\n", op, pc-4, r[24]);
				switch (op & 0x3f)
				{
					case 0:		// EMUL_RETURN
						runnging = false;
